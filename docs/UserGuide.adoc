...
image::UG_TaskDisplay.PNG[width="350"]
...
Encrypts XML files with a password so that unauthoried users cannot view the encrypted files without the password. +
Format: `encryptfiles PASSWORD`
Format: `decryptfiles PASSWORD`

****
* Encrypts the XML files with user-defined password
* Decrypts XML files if same password is provided
