...
. The user executes the `task-add` command and specify the required arguments.
. The `AddressBookParser` class creates a new `TaskAddCommandParser` class to parse the command arguments.
. The `TaskAddCommandParser` class parses the arguments and create a new `TaskAddCommand` class.
...
. The user executes the `task-delete` command and specify the required argument.
. The `AddressBookParser` class creates a new `TaskDeleteCommandParser` class to parse the command argument.
. The `TaskDeleteCommandParser` class parses the argument and create a new `TaskDeleteCommand` class.
...
. The user executes the `task-edit` command and specify the required arguments.
. The `AddressBookParser` class creates a new `TaskEditCommandParser` class to parse the command arguments.
. The `TaskEditCommandParser` class parses the arguments and create a new `TaskEditCommand` class.
...
. The user executes the `task-find` command and specify the required argument(s).
. The `AddressBookParser` class creates a new `TaskFindCommandParser` class to parse the command argument(s).
. The `TaskFindCommandParser` class parses the argument(s) and create a new `TaskFindCommand` class.
...
The sort mechanism sorts the tasks list based on priority.
It currently supports sorting in descending order of importance (i.e. high > medium > low).

The internal details of the sort mechanism is handled by the `sortByPriority` method in `UniqueTaskList` class.
The sort mechanism is implemented using a custom comparator in JavaFX ObservableList `sort` method.

The following sequence diagram shows how the sort tasks operation works.

image::TaskSortSequenceDiagram.png[width="800"]

. The user executes the `task-sort` command.
. The `AddressBookParser` class parses the command and create a new `TaskSortCommand` class.
. The `task-sort` command is executed and handled by the `ModelManager` class.
. The sorted list is returned and displayed to the user.

==== Design Considerations

===== Aspect: Implementation of `TaskSortCommand`

* **Alternative 1 (current choice):** Sort tasks by priority
** Pros: Easy to implement and does not need to make changes for every newly added field
** Cons: Lack flexibility in sorting by other fields
* **Alternative 2:** Sort tasks by other fields of a task such as name
** Pros: Allows flexibility in sorting
** Cons: Might be more difficult to implement as changes need to be made for every newly added field
// end::task-sort[]
...
=== Import/Export feature (since v1.1)

==== Current Implementation
Right now, only Export functionality is implemented.
...
image::ExportingSequenceDiagram.png[width="800"]

==== Design Considerations

===== Aspect: Usages of Exported Data

* **Alternative 1:** Allows option of exporting data into either CSV or XML files.
...
* **Alternative 2 (current choice):** Only allows exporting of data as XML files.
** Pros: Simplified export into a file that is readable and understandable by humans, is completely compatible with Java and 100% portable.
** Cons: Users may not know the benefits or uses of an XML file.

=== Calendar View feature (since v1.2)
...
All of the fields are compulsory, although the user can simply fill in a dummy detail.

The following sequence diagram shows how the add event feature works.

Diagram and steps to be included in future.
Pros, Cons, and Reasons to be added in future.
...
Allows easy deletion of an unwanted event according to the event index listed on the Event List in the app.
...
=== Profile Picture feature (since v1.3)
...
* manage contacts, tasks and events from one place

*Feature contribution*:

* Chan Yin Xin

Major: Implement task/to-do list feature to facilitate task management
...
Major: Calendar view feature to check summarised schedule for the month.

Minor: Export and Import features for address book portability to google contacts.
...
|`* * *` |traveler |add event to my calender|receive a reminder on those dates (eg. Birthdays, Meetings)

|`* * *` |traveler |edit event |so that it is up-to-date

|`* * *` |traveler |receive email notification |get reminded before the event is about to occur

|`* * *` |traveler |receive in-app notification |get reminder before the event is about to occur
...
|`* * *` |traveler and businessman |sort all tasks by priority  | easily view all the tasks by importance.
...
|`* *` |traveler |display a motivational code |keeps motivated and going
...
|`* *` |careful businessman|export data |to keep as emergency backup

|`* *` |impatient businessman |import data | to port over different devices easily
...
+
[none]
** 1b1. XML file is exported to Addressbook's main folder.
+
Use case ends.
...
[discrete]
=== Use case: Sort all tasks by priority

*MSS*

1.  User requests to sort all tasks in AddressBook
2.  AddressBook sorts the list of tasks by priority in decreasing order of importance (i.e. high > medium > low)
3.  AddressBook shows a list of sorted tasks
+
Use case ends.
