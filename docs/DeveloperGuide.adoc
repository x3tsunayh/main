...
`completionDate` is to store the date when the task is completed, it has a default date when a task is first created.
`timeLimit` is to store the number of days after the completionDate before a completed task is removed from the task list.
`markStatus` is for user to indicate if they want to continue keeping the completed task after the time limit
