...
The following sequence diagram shows how the add feature operation works.
...
The list task feature allows user to display all the tasks in the Address Book.
...
. The user executes the `addpicture INDEX FILENAME` command, with parameters `INDEX` and `FILENAME`
